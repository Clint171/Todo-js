...
router.post("/signup" , (req , res)=>{
    let confirm = service.saveUser(req.body);
...
router.put("/tasks/:email" , (req , res)=>{
    let email  = req.params.email;
    console.log(email + " updated task id: " + req.body.taskId);
    service.updateTask(email , req.body);
});

router.delete("/tasks/:email" , (req , res)=>{
    let email = req.params.email;
    console.log(email + " deleted task id: " + req.body.taskId);
    service.deleteTask(email , req.body.taskId).then(()=>{
        res.status(200);
        res.send("success");
    },
    ()=>{
