...
let createGroup = (req , res , next)=>{
    ...
    group.creationTime = group.creationDate.getTime();
    group.childGroups = [];
...
let getGroup = (req , res , next)=>{
    Group.findById(req.params.id).populate("members").populate("admins").populate("tasks").populate("events").populate("comments").populate("attachments").populate("joinRequests").populate("parentGroup").then(dbGroup =>{
        if(dbGroup && (dbGroup.owner == req.user._id || dbGroup.admins.includes(req.user._id) || dbGroup.members.includes(req.user._id))){
            res.status(200);
            res.send(dbGroup);
        }
    }).catch(err =>{
