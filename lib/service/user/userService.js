		...
		if (dbUser != null || dbUser != undefined) {
			...
			bcrypt.hash(user.password, 10, (err, hash) => {
				if (err) {
					...
					email.sendHtmlMail(
						req.body.defaultEmail,
						"Todo Email Verification",
						`<h1>Welcome Aboard!<h1><h3>We're glad you chose to join us.</h3><p> Click <a href="${process.env.URL}/verification/${user.defaultEmail}/${user.verificationCode}">HERE</a> to verify your account and begin your journey with us :)</p><br><p>Not you? Don't worry, someone might have mistyped their email. You can ignore or delete this email.</p>`
					);
...
let saveNewGoogleUser = (req, res, next) => {
	// decode the JWT token in the body
	...
	User.findOne({ defaultEmail: user.defaultEmail }).then((dbUser, err) => {
		if (dbUser != null || dbUser != undefined) {
			...
			if (decoded.email_verified) {
				user.verified = true;
				user.lastLoginUtcDate = new Date();
				user.save();
	...
	User.findById(req.user._id).then((dbUser) => {
		if (dbUser == null || dbUser == undefined) {
			...
			email.sendHtmlMail(
				...
				`<h1>Account Deleted!</h1><p>Your account has been deleted.</p><p>If you didn't delete your account, please contact us immediately.</p>`
			);
			res.status(200);
			...
			if (dbUser.verificationCodeExpirationUtcTimestamp > now.getTime()) {
				User.findOneAndUpdate(
					{ defaultEmail: req.params.email },
					{
						...
						verificationCodeExpirationUtcDate: null,
						verificationCodeExpirationUtcTime: null,
						verificationCodeExpirationUtcTimestamp: null,
					}
				).then(() => {
					res.status(200);
				...
				User.findOneAndUpdate(
					...
					res.send(
						"Verification code expired: " +
							dbUser.verificationCodeExpirationUtcTimestamp +
							" : " +
							now.getTime()
					);
				});
...
let resendVerification = (req, res, next) => {
	User.findOne({ defaultEmail: req.params.email }).then((dbUser) => {
		...
		dbUser.verificationCodeExpirationUtcDate =
			expirationDate.getUTCDate() +
			"/" +
			expirationDate.getUTCMonth() +
			"/" +
			expirationDate.getUTCFullYear();
		dbUser.verificationCodeExpirationUtcTime =
			expirationDate.getUTCHours() +
			":" +
			expirationDate.getUTCMinutes() +
			":" +
...
let searchUser = (req, res, next) => {
	User.find({ fullName: { $regex: req.params.query, $options: "i" } }).then(
		(dbUsers) => {
