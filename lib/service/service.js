...
let saveNewUser = async (req , res , next)=>{
    ...
    bcrypt.hash(user.password , 10 ,(err , hash)=>{
        if(err){
            ...
            email.sendHtmlMail(req.body.defaultEmail , "Todo Email Verification" , `<h1>Welcome Aboard!<h1><h3>We're glad you chose to join us.</h3><form method="POST" action="localhost:3000/verification/${user.defaultEmail}"><input type="number" hidden name="verificationCode" value="${verificationCode}"><input type="submit" value="Verify New Account"></form><br><p>Not you? Don't worry, someone might have mistyped their email. You can ignore or delete this email.</p>`);
            res.status(201);
...
let verifyNewUser = async (req , res , next)=>{
    let dbUser = db.getUser({defaultEmail : req.params.email});
    if(dbUser.verificationCode = req.body.verificationCode){
        if(dbUser.verificationCodeExpirationUtcTimestamp < Date.now()){
            ...
            res.status(401);
            res.send("Verification code expired!");
        }
    }
    else{
        res.status(401);
