    ...
    User.findOne({defaultEmail : req.params.email}).then(dbUser =>{
        if(dbUser.verificationCode == req.params.code){
            ...
            if(dbUser.verificationCodeExpirationUtcTimestamp > now.getTime()){
                User.findOneAndUpdate({defaultEmail : req.params.email} , {verified : true , status : "active" , verificationCode : null , verificationCodeExpirationUtcDate : null , verificationCodeExpirationUtcTime : null , verificationCodeExpirationUtcTimestamp : null});
                res.status(200);
                res.redirect("/login");
            }
            else{
                User.findOneAndUpdate({defaultEmail : req.params.email} , {verificationCode : null , verificationCodeExpirationUtcDate : null , verificationCodeExpirationUtcTime : null , verificationCodeExpirationUtcTimestamp : null});
                res.status(401);
                res.send("Verification code expired: "+ dbUser.verificationCodeExpirationUtcTimestamp +" : "+ now.getTime());
            }
        }
        else{
            User.findOneAndUpdate({defaultEmail : req.params.email} , {verificationCodeAttemts : {$inc : 1}});
            res.status(401);
